...
- `effect/fs.lzscr`: Filesystem helpers that wrap runtime `!fs.*` effects and expose ergonomic `Result`-first APIs (e.g. `read_text_*`).
...
| `effect/fs.lzscr` | Effect | Filesystem helpers that call `!fs.read_text` and return `Result` interfaces so callers decide when to surface errors or fallbacks. | Expand with write helpers (`write_text`, append) and directory utilities once runtime hooks land. |
