...
- Keep extending the `effect/fs` namespace that wraps the runtime filesystem effects (`!fs.read_text`, `!fs.write_text`, `!fs.append_text`, etc.) so every helper (`read_text_*`, `write_text_*`, `append_text_*`) funnels results through `Result`; next focus areas are directory helpers once runtime hooks land.
