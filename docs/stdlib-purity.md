...
- Keep extending the `effect/fs` namespace that wraps the runtime filesystem effects (`!fs.read_text`, `!fs.write_text`, `!fs.append_text`, `!fs.list_dir`, `!fs.remove_file`, `!fs.create_dir`, etc.) so every helper funnels results through `Result`; next focus areas are directory metadata helpers once runtime hooks land.
