...
- Keep extending the `effect/fs` namespace that wraps the runtime filesystem effects (`!fs.read_text`, `!fs.write_text`, `!fs.append_text`, `!fs.list_dir`, etc.) so every helper (`read_text_*`, `write_text_*`, `append_text_*`, `list_dir_*`) funnels results through `Result`; next focus areas are removal/metadata helpers once runtime hooks land.
