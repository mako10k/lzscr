...
//! # Span Normalization
//!
//! The diagnostic system applies normalization heuristics to improve error display:
//! - 1-character spans are expanded to the first meaningful token
//! - Leading `{` is shifted inward to the first non-whitespace token
//! - Comment lines are skipped when finding meaningful positions
//!
//! Note: Actual normalization is performed by the CLI's SourceRegistry during
//! display, as it requires access to the full source text and module information.
...
/// Origin of a diagnostic span.
///
/// Tracks whether the span came from user code or was generated by the compiler.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SpanOrigin {
    /// Span directly from user source code
    Source,
    /// Span generated during inference (e.g., for type variables)
    Generated,
}
...
pub struct DiagnosticSpan {
    ...
    /// Span origin (source or generated)
...
impl DiagnosticSpan {
    ...
    pub fn generated(offset: usize, len: usize) -> Self {
        Self {
            offset,
            len,
            label: None,
            context: SpanContext::Primary,
            origin: SpanOrigin::Generated,
        }
    }
